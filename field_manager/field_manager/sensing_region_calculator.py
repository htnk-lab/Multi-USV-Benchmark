...
class SensingRegionCalculator(Node):
    ...
    def __init__(self) -> None:
        super().__init__("sensing_region_calculator")
        ...
        self.point_density = float(np.prod(field_generator.grid_span))
    ...
    def curr_pose_array_callback(self, msg: PoseArray) -> None:
        ...
        _centroid, _grid_points, sensing_region = self.calc_voronoi_tesselation(msg.poses)
    ...
    def calc_voronoi_tesselation(self, pose_list: List[Pose]) -> Tuple[NDArray, List[NDArray], NDArray]:
        # extract agent positions as NDArrays
        all_agent_position_list: List[NDArray] = [
            np.array([pose.position.x, pose.position.y]) for pose in pose_list
        ]
        ...
        neighbor_agent_position_list = [
            pos for i, pos in enumerate(all_agent_position_list) if i != self.agent_id
        ]

        # When using a circular sensor model, this corresponds to the optimal solution
        # when adopting h_{1} as the performance function according to K.Sugimoto et al. 2015
